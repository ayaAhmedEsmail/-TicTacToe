...
import static tictactoe.OnlineUsersBase.stage;
...
public class HistoryBase extends Pane {
    ...
    protected static Label winsLabel;
    protected static Label lossesLabel;
    protected final Button backBtn;
    protected static ListView Player1list;
    protected static ListView Player2list;
    protected static ListView Winnerlist;
    ...
    public HistoryBase(Stage s, String name) {
        stage = s;
        label = new Label();
        label0 = new Label();
        label1 = new Label();
        ...
        DataAccessLayer.printGamesToList(name);
        DataAccessLayer.setWinsAndLosses(name);
        backBtn.setOnAction((event) -> {
            Parent newRoot = new OnlineUsersBase(stage, name);
            Scene scene = new Scene(newRoot);
            stage.setScene(scene);
        });
        javafx.application.Platform.runLater(// Avoid throwing IllegalStateException by running from a non-JavaFX thread.
                () -> {
                    Node n1 = Player1list.lookup(".scroll-bar");
                    if (n1 instanceof ScrollBar) {
                        final ScrollBar bar1 = (ScrollBar) n1;
                        Node n2 = Player2list.lookup(".scroll-bar");
                    ...
                    if (n1 instanceof ScrollBar) {
                        final ScrollBar bar1 = (ScrollBar) n1;
                        Node n3 = Winnerlist.lookup(".scroll-bar");
                        if (n3 instanceof ScrollBar) {
                            final ScrollBar bar2 = (ScrollBar) n3;
                            bar1.valueProperty().bindBidirectional(bar2.valueProperty());
                        }
